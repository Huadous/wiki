...
bthread 的线程个数是软限而非硬限，在 brpc server 端默认在 bthread 中处理请求。当某个请求发生 [[concepts/超时|超时]] 时，只阻塞所在的 bthread，并不会影响为新请求建立新的 bthread。这与传统 [[concepts/线程池|线程池]] 设计形成鲜明对比：拥塞时传统服务的最大 [[concepts/QPS|QPS]] 跳变是因为线程个数是硬限，单个请求的处理时间很大程度上决定了最大 QPS，而 brpc 的 bthread 模型能避免因超时阻塞线程导致最大 QPS 骤降的问题，是其在抗雪崩方面的核心优势。

## 关键实体

- [[entities/bthread|bthread]] — Apache brpc 项目的 M:N 线程库，前身为百度 DP 项目中的 fiber，数百纳秒即可建立，线程个数为软限
- [[entities/brpc|brpc]] — Apache 开源 RPC 框架，bthread 是其核心线程库，server 端默认在 bthread 中处理请求
...
- [[concepts/线程池|线程池]] — 传统服务中线程个数为硬限的并发模型，超时会阻塞线程导致 QPS 骤降
- [[concepts/超时|超时]] — bthread 模型下单个请求超时仅阻塞所在 bthread，不影响为新请求建立 bthread
- [[concepts/QPS|QPS]] — 衡量服务并发能力的指标，bthread 模型在拥塞场景下能保持较高 QPS
- [[concepts/拥塞|拥塞]] — 服务压力大时的场景，bthread 软限设计在此场景下展现抗雪崩优势
...
- bthread 的线程个数是软限而非硬限，与传统 [[concepts/线程池|线程池]] 固定线程数设计不同
- brpc server 端默认在 bthread 中处理请求，单个请求超时只阻塞所在 bthread，不影响为新请求建立新 bthread
- 在拥塞场景下，传统服务因线程硬限而 QPS 骤降，brpc 的 bthread 软限模型能保持较高服务并发度，具备抗雪崩能力