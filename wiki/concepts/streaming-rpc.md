...
从客户端视角来看，Streaming RPC 是一种用于传输流式数据的"应用层连接"（application-level connection）。[[sources/client|client]] 与 [[sources/en_client|en_client]] 均明确指出："brpc 支持 Streaming RPC，这是一种应用层的连接，用于传递流式数据。"需要强调的是，这里的"应用层连接"与 brpc 客户端所支持的 [[concepts/Connection Type|Connection Type]] 中的 single、pooled、short 等连接模型并不在同一层次——后者是传输层连接方式，前者是在传输层连接之上构建的应用层流式通信模式。Streaming RPC 适用于需要持续传输大量数据或实时通信的场景，并且可以与各种传输层连接方式配合使用，brpc 客户端文档以单独链接 `streaming_rpc.md` 的方式把它作为独立特性介绍。
...
> - "Check out [Streaming RPC](streaming_rpc.md) for more details on this application-level connection."
