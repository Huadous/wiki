...
从客户端视角来看，Streaming RPC 是一种用于传输流式数据的"应用层连接"（application-level connection），与 brpc 客户端所支持的 [[concepts/Connection Type|Connection Type]] 中的 single、pooled、short 等连接模型并列，[[sources/en_client|en_client]] 明确将其定位为面向持续或大体量数据流场景而设计的连接形态，而非离散的请求-响应交互。brpc 客户端文档以单独链接 `streaming_rpc.md` 的方式把它作为独立特性介绍，说明它拥有专属的协议语义与使用方式。
